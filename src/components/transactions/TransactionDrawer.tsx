    ...
    <Drawer open={open} onOpenChange={next => (next ? onOpenChange(true) : closeDrawer())}>
      <DrawerContent className="[--drawer-inset:0.5rem] [--bleed:0px] rounded-xl">
        <div className="mx-auto w-full">
          ...
          {/* The form reads the wallets, which suspends. It lives inside the
              content, which is unmounted while closed, so the read happens
              when the drawer opens rather than when the shell does. */}
          <PopupBoundary>
            <TransactionDrawerBody
              ...
              onSubmit={onSubmit}
              onEntrySaved={() => setEntryCount(count => count + 1)}
              onClose={closeDrawer}
              onDeleteRequest={transaction && onDelete ? () => setShowDeleteConfirm(true) : undefined}
            />
          </PopupBoundary>
        </div>
      </DrawerContent>
...
interface TransactionDrawerBodyProps {
  transaction?: Transaction | null
  initialValues?: Partial<CreateTransaction>
  entryCount: number
  onSubmit: (data: CreateTransaction) => Promise<void>
