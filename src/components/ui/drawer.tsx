...
// Project customizations, all overwritten by re-running `shadcn add drawer`:
//  - drawers come up from the bottom on mobile and in from the right on
//    desktop, with the swipe handle only where it can be swiped. Pass
//    `swipeDirection` / `showSwipeHandle` explicitly to opt out.
//  - DrawerHeader keeps its bottom padding and DrawerFooter its top padding.
//    Upstream drops both, expecting the content in between to supply its own;
//    ours does not, so the content would touch the header and footer.
function Drawer({
  modal = true,
  showSwipeHandle,
  snapPoints,
  ...
  const resolvedDirection = swipeDirection ?? (isMobile ? "down" : "right")
  const resolvedHandle = showSwipeHandle ?? isMobile
  ...
  const contextValue = React.useMemo(
    () => ({
      hasSnapPoints,
      modal,
      showSwipeHandle: resolvedHandle,
      swipeDirection: resolvedDirection,
    }),
    [hasSnapPoints, modal, resolvedHandle, resolvedDirection]
  )
